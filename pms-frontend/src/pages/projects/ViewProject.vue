<template>
  <div class="page-shell" :class="{ 'is-dark': isDarkMode }">
    <div class="page-panel">

      <!-- Loading -->
      <div v-if="loading" class="loading-state">
        <div class="spinner-lg" /><span>Loading project…</span>
      </div>
      ...
      <template v-else-if="project">
        ...
        <div class="hero" :style="{ background: heroGradient }">
          <div class="hero-top">
            ...
            <div class="hero-actions">
              <button class="h-btn" @click="goBack" title="Back">
                <ArrowLeftIcon class="icon" />
              </button>
              <button v-if="canEditProjectAction" class="h-btn" @click="openEdit" title="Edit">
                <EditIcon class="icon" />
              </button>
            </div>
          </div>
          ...
          <div class="hero-meta">
            <span class="h-pill" v-if="project.current_stage">
              <LayersIcon class="pi" />{{ project.current_stage.name }}
            </span>
            <span class="h-pill status-pill" :style="heroStatusStyle" v-if="project.status">
              <span class="sdot" />{{ project.status.name }}
            </span>
            <span class="h-pill" v-if="project.project_type">
              <BriefcaseIcon class="pi" />{{ project.project_type.name }}
            </span>
          </div>
          <div v-if="project.progress_percentage !== undefined" class="hero-prog">
            <div class="hp-track">
              <div class="hp-fill" :style="{ width: `${project.progress_percentage}%` }" />
            </div>
        ...
        <div class="tab-body">
          ...
          <div v-show="activeTab === 'overview'" class="tab-pane">
            ...
            <div class="two-col">
              <div class="info-card">
                ...
                <div class="d-list">
                  <div v-if="project.project_type" class="d-item">
                    <span class="dl">Type</span><span class="dv">{{ project.project_type.name }}</span>
                  </div>
                  <div v-if="project.industry" class="d-item">
                    <span class="dl">Industry</span><span class="dv">{{ project.industry.name }}</span>
                  </div>
                  <div v-if="project.sector" class="d-item">
                    <span class="dl">Sector</span><span class="dv">{{ project.sector.name }}</span>
                  </div>
                  <div v-if="project.investment_type" class="d-item">
                    <span class="dl">Investment</span><span class="dv">{{ project.investment_type.name }}</span>
                  </div>
                </div>
              </div>
              <div class="info-card">
                ...
                <div class="d-list">
                  <div v-if="project.proposal_date" class="d-item">
                    <span class="dl">Proposed</span><span class="dv">{{ fmtDate(project.proposal_date) }}</span>
                  </div>
                  <div v-if="project.start_date" class="d-item">
                    <span class="dl">Started</span><span class="dv">{{ fmtDate(project.start_date) }}</span>
                  </div>
                  <div v-if="project.target_completion_date" class="d-item">
                    <span class="dl">Target</span>
                    <span class="dv" :class="{ 'ov-text': project.is_overdue }">
                      {{ fmtDate(project.target_completion_date) }}
                    </span>
                  </div>
                  <div v-if="project.actual_completion_date" class="d-item">
                    <span class="dl">Completed</span>
                    <span class="dv ok-text">{{ fmtDate(project.actual_completion_date) }}</span>
                  </div>
                </div>
              </div>
            </div>
            <div v-if="project.estimated_cost || project.actual_cost" class="info-card">
              ...
              <div class="fin-grid">
                <div class="fin-item">
                  <span class="fl">Estimated</span>
                  <span class="fa">{{ fmtCur(project.estimated_cost || 0, project.currency) }}</span>
                </div>
                <div v-if="project.actual_cost" class="fin-item">
                  <span class="fl">Actual</span>
                  <span class="fa">{{ fmtCur(project.actual_cost, project.currency) }}</span>
                </div>
                <div v-if="project.funding_source" class="fin-item">
                  <span class="fl">Funding</span>
                  <span class="fa sm">{{ project.funding_source.name }}</span>
                </div>
                <div v-if="project.estimated_cost && project.actual_cost" class="fin-item">
                  <span class="fl">Variance</span>
                  <span
                    class="fa"
                    :class="project.actual_cost > project.estimated_cost ? 'neg' : 'pos'"
                  >
                    {{ fmtCur(project.actual_cost - project.estimated_cost, project.currency) }}
                  </span>
                </div>
              </div>
            </div>
            <div class="two-col">
              <div v-if="project.location_address" class="info-card">
                ...
                <div
                  v-if="project.location_lat && project.location_lng"
                  class="coord-chip"
                >
                  {{ Number(project.location_lat).toFixed(4) }},
                  {{ Number(project.location_lng).toFixed(4) }}
                </div>
              </div>
              <div v-if="project.proponent_name" class="info-card">
                ...
                <div class="d-list">
                  <div class="d-item">
                    ...
                    <span class="dv">{{ project.proponent_name }}</span>
                  </div>
                  <div v-if="project.proponent_contact" class="d-item">
                    <span class="dl">Contact</span>
                    <span class="dv">{{ project.proponent_contact }}</span>
                  </div>
                  <div v-if="project.proponent_email" class="d-item">
                    <span class="dl">Email</span>
                    <a :href="`mailto:${project.proponent_email}`" class="dv link">
          ...
          <div v-show="activeTab === 'team'" class="tab-pane">
            <div class="pane-head">
              <h3>Team Members</h3>
              <button v-if="canManageMembersAction" class="add-btn" @click="openAddMember">
                <UserPlusIcon class="icon" /> Add Member
              </button>
            </div>
            <div v-if="activeMembers.length > 0" class="members-list">
              <div v-for="m in activeMembers" :key="m.id" class="member-card">
                ...
                <div v-if="canManageMembersAction" class="m-actions">
            ...
            <div v-else class="empty-pane">
              <UsersIcon class="ep-icon" /><p>No team members added yet</p>
            </div>
          </div>
          ...
          <div v-show="activeTab === 'timeline'" class="tab-pane">
            <div v-if="timelineLoading" class="tl-loading">
              <div class="spinner-sm" /> Loading history…
            </div>
            <div v-else-if="timelineData">
                ...
                <div class="tl-items">
                  <div v-for="h in timelineData.stage_history" :key="h.id" class="tl-item">
                    ...
                    <div class="tl-content">
                      <p class="tl-text">
                        Stage changed<span v-if="h.from_stage" class="from"> from {{ h.from_stage.name }}</span>
                        to <strong>{{ h.to_stage?.name }}</strong>
                      </p>
              ...
              <div v-if="timelineData.status_history.length > 0" class="tl-section">
                ...
                <div class="tl-items">
                  <div v-for="h in timelineData.status_history" :key="h.id" class="tl-item">
                    ...
                    <div class="tl-content">
                      <p class="tl-text">
                        Status changed<span v-if="h.from_status" class="from"> from {{ h.from_status.name }}</span>
                        to <strong>{{ h.to_status?.name }}</strong>
                      </p>
                      <p v-if="h.change_reason" class="tl-reason">{{ h.change_reason }}</p>
                      <p class="tl-meta">{{ fmtDate(h.changed_at) }} · {{ h.changed_by_user?.name }}</p>
                    </div>
              ...
              <div v-if="!timelineData.stage_history.length && !timelineData.status_history.length" class="empty-pane">
                <ClockIcon class="ep-icon" /><p>No history available</p>
              </div>
            </div>
          </div>
    ...
    <!-- Member modal — kept as a modal-on-top of the page -->
    <Teleport to="body">
      <Transition name="modal">
        <div
          v-if="showMemberModal"
          class="modal-overlay member-overlay"
        ...
        >
          <div class="member-modal">
            ...
            <div class="member-foot">
              ...
              <button class="add-btn" @click="saveMember">
...
import {
  X as XIcon, ArrowLeft as ArrowLeftIcon, Edit as EditIcon,
  Layers as LayersIcon, Briefcase as BriefcaseIcon, FileText as FileTextIcon,
  Info as InfoIcon, Calendar as CalendarIcon, DollarSign as DollarSignIcon,
  MapPin as MapPinIcon, User as UserIcon, Users as UsersIcon, UserPlus as UserPlusIcon,
  Clock as ClockIcon, CheckCircle as CheckCircleIcon, ArrowRight as ArrowRightIcon,
...
const currentMember   = computed(() => activeMembers.value.find((m) => m.user_id === currentUserId.value))
...
const availableUsers = computed(() => {
  if (editingMemberId.value) return users.value
...
// ── Hero gradient / status pill ─────────────────────────────────────────
const heroGradient = computed(() => {
  if (project.value?.is_archived) return 'linear-gradient(135deg,#334155 0%,#0f172a 100%)'
  const m: Record<string, string> = {
    Active:    'linear-gradient(135deg,#0f4c81 0%,#0f172a 100%)',
    'On Hold': 'linear-gradient(135deg,#78350f 0%,#0f172a 100%)',
    Completed: 'linear-gradient(135deg,#1e3a5f 0%,#0f172a 100%)',
    Cancelled: 'linear-gradient(135deg,#7f1d1d 0%,#0f172a 100%)',
  }
...
const heroStatusStyle = computed(() => {
  const m: Record<string, { bg: string; color: string }> = {
    Active:    { bg: 'rgba(34,197,94,0.2)',  color: '#86efac' },
    'On Hold': { bg: 'rgba(245,158,11,0.2)', color: '#fcd34d' },
    Completed: { bg: 'rgba(59,130,246,0.2)', color: '#93c5fd' },
...
const loadProject = async () => {
  ...
  loading.value = true
  try { project.value = await projectStore.fetchProject(projectId.value) }
...
const loadTimeline = async () => {
  ...
  try { timelineData.value = await projectStore.fetchTimeline(projectId.value) }
...
watch(activeTab, async (tab) => {
  if (tab === 'timeline' && projectId.value && !timelineData.value) await loadTimeline()
  if (tab === 'team' && users.value.length === 0) await loadUsers()
})

// Re-load when navigating between view pages without unmount (e.g. /projects/1 → /projects/2).
...
const saveMember = async () => {
  ...
  try {
    await projectStore.addMember(projectId.value, {
...
const openEdit  = () => projectId.value && router.push(`/projects/edit/${projectId.value}`)

// ── Helpers ─────────────────────────────────────────────────────────────
const initials = (n: string) => n.split(' ').map(x => x[0]).slice(0, 2).join('').toUpperCase() || '?'
const fmtCur   = (a: number, cur = 'PHP') =>
  new Intl.NumberFormat('en-PH', { style: 'currency', currency: cur, maximumFractionDigits: 0 }).format(a)
const fmtDate  = (d: string) =>
  new Date(d).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
</script>

<style scoped>
/* ── Page shell — replaces the modal overlay ────────────────────────── */
.page-shell {
  --v-bg: #ffffff;
  --v-border: #e2e8f0;
  --v-sub: #f8fafc;
...
/* Hero — unchanged from the original modal */
...
/* Tab body */
.tab-body { padding: 1.25rem; overscroll-behavior: contain; }
...
/* Member modal — kept floating on top */
.modal-overlay { position: fixed; inset: 0; z-index: 9999; background: rgba(15,23,42,0.65); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; padding: 1rem; overflow-y: auto; }
.modal-overlay.is-dark { background: rgba(0,0,0,0.75); }
...
.h-close { width: 2.125rem; height: 2.125rem; display: flex; align-items: center; justify-content: center; border: 1px solid var(--v-border); background: transparent; border-radius: 0.5rem; cursor: pointer; color: var(--v-text-3); transition: all 0.15s; }
.h-close:hover { background: var(--v-muted); color: var(--v-text); }
...
/* Member modal transition */
.modal-enter-active { animation: ovIn 0.22s ease; }
.modal-leave-active { animation: ovIn 0.18s ease reverse; }
@keyframes ovIn { from{opacity:0} to{opacity:1} }
.modal-enter-active .member-modal { animation: panIn 0.28s cubic-bezier(0.34,1.4,0.64,1); }
.modal-leave-active .member-modal { animation: panIn 0.18s ease reverse; }
