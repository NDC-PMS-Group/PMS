<template>
  <main class="tasks-workspace">
    <header class="workspace-head">
      <div class="heading"><span>{{ isProjectRoute ? 'Project workspace' : 'My workspace' }}</span><h1>{{ pageTitle }}</h1><p>{{ isProjectRoute ? 'SOI work plan and delivery checklist' : 'Tasks across projects you can access' }}</p></div>
    ...
    <template v-else>
      ...
      <TaskChecklist v-else :tasks="tasks" :pagination="pagination" :grouped="isProjectRoute" @open="openTask" @page="changePage" />
    </template>
...
const pageTitle = computed(() => {
  if (!isProjectRoute.value) return "Tasks";
  const project = facets.value.projects.find((item) => item.id === projectId.value);
  return project?.label?.replace(/^.*? - /, "") || `Project ${projectId.value} Tasks`;
});
...
const toggleSubtask = async (subtask: TaskItem) => { try { await store.updateTask(subtask.id, { status: subtask.status === "completed" ? "pending" : "completed" }); if (selectedTask.value) selectedTask.value = await store.fetchTask(selectedTask.value.id); } catch (error: any) { toast.error(store.errorMessage(error, "Checklist item could not be updated.")); } };
