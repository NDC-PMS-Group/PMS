<template>
  <section class="filters" :class="{ open }">
    ...
    <div v-if="open" id="task-filters" class="filter-grid">
      ...
      <label>SOI phase<select :value="filters.soi_section || ''" @change="patch('soi_section', ($event.target as HTMLSelectElement).value || undefined)"><option value="">All phases</option><option v-for="item in facets.soi_sections" :key="item.value" :value="item.value">{{ formatLabel(item.value) }} ({{ item.count }})</option></select></label>
...
const activeCount = computed(() => [props.filters.project_id, props.filters.status, props.filters.priority, props.filters.soi_section, props.filters.assigned_to, props.filters.overdue, props.filters.urgent].filter(Boolean).length);
